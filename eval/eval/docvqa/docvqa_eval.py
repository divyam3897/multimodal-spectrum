...
def process_cambrian(line, wrong_line, wrong_line2, args, tokenizer, image_processor, model_config):
...
def process_qwen_llava(line, wrong_line, wrong_line2, args, tokenizer, image_processor, model_type):
    qs = wrong_line["question"] if args.text_shuffle else line["question"]
    qs += f"\n{args.question_extension}"
...
def process(line, wrong_line, wrong_line2, args, tokenizer, image_processor, model_config, model_type):
    if model_type in ['qwen2_5', 'qwen3', 'llava-next']:
        return process_qwen_llava(line, wrong_line, wrong_line2, args, tokenizer, image_processor, model_type)
    else:  # cambrian
        return process_cambrian(line, wrong_line, wrong_line2, args, tokenizer, image_processor, model_config)
...
def eval_model(args):
    ...
    for line, wrong_line, wrong_line2 in tqdm(zip(questions, shuffle_questions, shuffle_questions2), total=len(questions)):
        ...
        inputs, image_tensor, image_sizes, prompt = process(line, wrong_line, wrong_line2, args, tokenizer, image_processor, model.config, args.model_type)
        ...
        with torch.inference_mode():
            if args.model_type == 'cambrian':
                ...
                generated_ids = model.generate(
                    **inputs,
                    max_new_tokens=args.max_new_tokens,
                    do_sample=False if args.model_type in ['qwen2_5', 'qwen3'] else (True if args.temperature > 0 else False),
                    num_beams=1 if args.model_type in ['qwen2_5', 'qwen3'] else args.num_beams,
                    temperature=None if args.model_type in ['qwen2_5', 'qwen3'] else (args.temperature if args.temperature > 0 else None),
                    use_cache=True,
                    pad_token_id=tokenizer.pad_token_id
                )
