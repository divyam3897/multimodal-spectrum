...
def process_cambrian(line, wrong_line1, wrong_line2, args, tokenizer, image_processor, model_config, images):
...
def process_qwen_llava(line, wrong_line1, wrong_line2, args, tokenizer, image_processor, images, model_type):
    qs = wrong_line1["question"] if args.text_shuffle else line["question"]
    qs += f"\n{args.question_extension}"
...
def process(line, wrong_line1, wrong_line2, args, tokenizer, image_processor, model_config, images, model_type):
    if model_type in ['qwen2_5', 'qwen3', 'llava-next']:
        return process_qwen_llava(line, wrong_line1, wrong_line2, args, tokenizer, image_processor, images, model_type)
    else:  # cambrian
...
def eval_model(args):
    ...
    for line, wrong_line1, wrong_line2 in tqdm(zip(questions, shuffle_questions1, shuffle_questions2), total=len(questions)):
        ...
        inputs, image_tensor, image_sizes, prompt = process(line, wrong_line1, wrong_line2, args, tokenizer, image_processor, model.config, images, args.model_type)
        ...
        with torch.inference_mode():
            if args.model_type == 'cambrian':
                ...
                if args.model_type == 'qwen3':
                    # Qwen3 models: Use recommended generation parameters
                    # Reference: https://huggingface.co/Qwen/Qwen3-VL-8B-Instruct
                    # Recommended: greedy=false, top_p=0.8, top_k=20, temperature=0.7, repetition_penalty=1.0
                    generated_ids = model.generate(
                        **inputs,
                        max_new_tokens=args.max_new_tokens,
                        do_sample=True,  # greedy=false means do_sample=True
                        temperature=0.7,  # Default for Qwen3
                        top_p=0.8,  # Default for Qwen3
                        top_k=20,  # Default for Qwen3
                        repetition_penalty=1.0,  # Default for Qwen3
                        use_cache=True,
                        pad_token_id=tokenizer.pad_token_id
                    )
                elif args.model_type == 'qwen2_5':
                    # Qwen2.5 models: Use standard generation
                    generated_ids = model.generate(
                        **inputs,
                        max_new_tokens=args.max_new_tokens,
                        do_sample=False,
