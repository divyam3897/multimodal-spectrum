...
def calculate_metrics_from_file(jsonl_file):
    ...
    for ed in ed_range:
        combined_data['accuracy_edit_distance_'+str(ed)] = {'accuracy': 100.0 * get_accuracy(full_list, ed), 'total': len(full_list)}
...
def save_comparison_results(all_results: dict, output_dir: str):
    ...
    json_output_path = os.path.join(output_dir, f"synthdog_comparison_{model_slug}.json")
