...
def eval_model(args):
    ...
    print(
        "Here, text shuffling is",
        str(args.text_shuffle),
        "while image shuffling is",
    ...
    for line, wrong_line1, wrong_line2 in tqdm(
        ...
        used_question_id = qa_source["question_id"]
        used_image_id = img_line["question_id"]

        category = "val" if "val" in orig_question_id else "test"
        ...
        ans_file.write(
            json.dumps(
                {
                    "idx": idx,
                    "question_id": orig_question_id,
                    "used_question_id": used_question_id,
                    "image_id": orig_question_id + ".jpg",
                    "used_image_id": used_image_id + ".jpg",
                    "prompt": prompt,
                    "answer": outputs,
                    "gt_answer": gt_answer,
                    "model_id": model_name,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--text_shuffle",
        action="store_true",
        help="Enable text shuffle (question+answer semantics from another sample)",
    )
    parser.add_argument(
        "--image_shuffle",
        action="store_true",
        help="Enable image shuffle (image from another sample)",
    )
