...
def _resolve_image_from_sample(sample):
    ...
    return Image.open(file_path).convert("RGB"), image_path
...
def process_cambrian(line, wrong_line1, wrong_line2, args, tokenizer, image_processor, model_config):
    ...
    input_image, used_image_path = _resolve_image_from_sample(img_source)
...
def eval_model(args):
    ...
    for line, wrong_line1, wrong_line2 in tqdm(
        ...
        inputs, image_tensor, image_sizes, prompt, img_source, used_image_path = process(
            ...
            json.dumps(
                {
                    ...
                    "used_image": used_image_path,
                    "orig_image": line["image"],
                }
            )
            + "\n"
        )
